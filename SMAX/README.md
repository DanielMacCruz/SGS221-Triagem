...
### Passo 2: Ativar Modo Desenvolvedor (Opcional/Recomendado)
Para garantir que o script rode sem bloqueios em alguns navegadores:
...
Acesse o arquivo do script no repositório (arquivo `.user.js`) e clique em **Raw** ou **Instalar**.
- **Automático:** O Tampermonkey deve abrir uma aba pedindo confirmação. Clique em **Instalar**.
- **Manual:** Se não abrir, copie todo o código, vá no Tampermonkey -> **+ (Criar novo script)**, cole o código e salve (`Ctrl+S`).
...
O sistema permite gerenciar múltiplas equipes (ex: **JEC**, **Vara Comum**) e definir regras automáticas de sugestão baseadas no **Local de Divulgação** ou **Grupo de Suporte (GSE)** do chamado.

1. **Criar Equipe:** Clique em `+ Nova Equipe`.
2. **Definir Regras de Roteamento:**
   - **GSE:** Adicione grupos que pertencem a esta equipe.
   - **Local de Divulgação:** Adicione termos (ex: "JUIZADO ESPECIAL") para que chamados com esse local sejam sugeridos automaticamente para esta equipe.
3. **Adicionar Membros:**
...
- **Modo Real:** O script já opera em modo real por padrão (gravação direta no SMAX).

---

## 3. HUD de Triagem (Interface Principal)

Clique em **INICIAR TRIAGEM** (botão flutuante azul) para abrir o HUD.

### Cabeçalho e Filtros
O topo do HUD foi organizado para facilitar o fluxo de leitura da esquerda para a direita:
1. **Local de Divulgação:** Mostra a orígem do chamado (ex: "JUIZADO DA FAZENDA PUBLICA").
2. **Meus Finais:** Define quais chamados você vai processar. Ex: `00-50` (filtra apenas esses finais).
...
- **Anexos:** Exibidos no canto inferior direito. Clique para visualizar sem baixar.
...
## 4. Dicas de Uso
- **Atalhos:** Use as setas do teclado (se habilitado) para navegar rápido.
